...
- **WezTerm nightly** with a flat tab bar (each tab labelled `N: <dir>`, tinted green when Claude finishes / red on error), integrated window buttons (minimize/maximize/close), JetBrainsMono Nerd Font at 11.5pt, active workspace · cwd in the right-status, vim-style splits (`Ctrl+Space h`/`v` local; `Ctrl+Space H`/`V` to split a chosen SSH/WSL domain), `Alt+1…9` tab selection, `Ctrl+V` rebound for synthetic-paste compatibility (Wispr Flow, etc.), `Ctrl+Space o` to pop the current pane into its own window, and workspace management (`Ctrl+Space R` rename, `Ctrl+Space X` close-all). On macOS, two System Settings toggles are needed first to free `Ctrl+Space` and the F-row from the OS — see `INSTALL.md` § macOS.
...
- **Modern CLI tools**: eza, zoxide, fzf, bat, git-delta, ripgrep, and the `micro` editor (a friendly nano alternative) — installed on both sides, with delta wired into `git diff` and the stack's `git st/lg/lga/br/co/cm` aliases via a managed gitconfig include.
