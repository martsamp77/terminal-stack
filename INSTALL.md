...
The choco path installs WezTerm **stable** (Chocolatey has no nightly channel); the config detects the build and degrades gracefully (plain resize border instead of integrated buttons, no-op `LEADER o`), so it loads cleanly on the stable build. Every other package matches the winget list. winget stays the default whenever `-PackageManager` / `$env:TERMINAL_STACK_PKGMGR` is unset, so the Windows 11 path is unaffected. On a headless server the WezTerm GUI config deploys but is unused — the Starship prompt, CLI tools, pwsh `$PROFILE`, and Claude Code hooks all work without it.

> **Running WezTerm over RDP?** There's no usable GPU, so the default `OpenGL` front-end fails with *"OpenGL implementation is too old to work with glium"* and the GUI never starts. Set `WEZTERM_FRONT_END=WebGpu` — it falls back to the WARP software D3D adapter (Microsoft Basic Render Driver) and works. (`Software` does **not** help on Windows; the stable build routes it through OpenGL too.) Persist it machine-wide from an elevated session: `[Environment]::SetEnvironmentVariable('WEZTERM_FRONT_END','WebGpu','Machine')`, then relaunch.
