...
> **Running WezTerm over RDP?** There's no usable GPU context, so the default `OpenGL` front-end silently fails to start. Set a machine env var `WEZTERM_FRONT_END=Software` (e.g. `[Environment]::SetEnvironmentVariable('WEZTERM_FRONT_END','Software','Machine')` from an elevated session) and relaunch.
