...
- **`WEZTERM_FRONT_END` env override.** `front_end` still defaults to `OpenGL`, but reads `$env:WEZTERM_FRONT_END` first, so a host with no usable GPU context (over RDP / headless server, where the GUI otherwise silently never starts) can opt into `'Software'` machine-wide without editing tracked config — the WezTerm analogue of `~/.zshrc.local`. See `docs/decisions.md` § "Why the WezTerm config tolerates the 20240203 stable".
