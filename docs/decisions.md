...
Separately, `front_end` defaults to `OpenGL` (the WebGpu output-buffer-stall fix on some Intel iGPUs — see "Why OpenGL front_end…"), but **over RDP or on a headless server there is no usable GPU context and the GUI won't start at all**. So `front_end` reads `os.getenv('WEZTERM_FRONT_END')`, letting such a host opt into `'Software'` machine-wide without editing tracked config — the WezTerm analogue of the `~/.zshrc.local` per-machine seam. This was found the hard way: on a Windows Server 2019 box the GUI silently failed to launch (no `%LOCALAPPDATA%\wezterm` log even created) until `WEZTERM_FRONT_END=Software`.
