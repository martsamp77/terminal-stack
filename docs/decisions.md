...
## Why the WezTerm config tolerates the 20240203 stable (and how `front_end` is overridden on servers)

The stack prefers WezTerm nightly, but winget-less hosts get the only `wezterm` Chocolatey package — the last *stable*, `20240203` (Feb 2024). Rather than force a manual nightly download on those hosts, the config **degrades gracefully** so the stable build starts cleanly:

- `window_decorations` uses `INTEGRATED_BUTTONS|RESIZE` only when `wezterm.version`'s build date is newer than `20240203`, else plain `RESIZE`. `INTEGRATED_BUTTONS` doesn't exist on the stable build, and an invalid `window_decorations` value is the kind of thing that surfaces a config-error overlay; the date gate (`tonumber(wezterm.version:match('^(%d+)'))`) avoids it. Nightly keeps the integrated min/max/close buttons.
- The `LEADER o` detach-tab binding calls `pane:move_to_new_window()`, also nightly-only; the binding is gated on the same `build_date` so it no-ops on stable instead of erroring at keypress.

Everything else in the config (font, Catppuccin, fancy tab bar, leader/pane keys, `format-tab-title`, `launch_menu`) predates `20240203`, so the stable build is fully usable — just without the two nightly niceties.

Separately, `front_end` defaults to `OpenGL` (the WebGpu output-buffer-stall fix on some Intel iGPUs — see "Why OpenGL front_end…"), but **over RDP or on a headless server there is no usable GPU and `OpenGL` fails outright** with *"OpenGL implementation is too old to work with glium"* (RDP exposes only GL 1.1; glium needs 3.x), so the GUI never starts. `front_end` therefore reads `os.getenv('WEZTERM_FRONT_END')`, letting such a host override per-machine without editing tracked config — the WezTerm analogue of the `~/.zshrc.local` seam. The value that works on Windows over RDP is **`WebGpu`**, *not* `Software`: on the `20240203` stable, `Software` is implemented over OpenGL too and hits the same glium wall, whereas `WebGpu` falls back to the WARP software D3D adapter (Microsoft Basic Render Driver, Dx12) and renders fine. Found the hard way on a Windows Server 2019 box — the GUI silently failed (no `%LOCALAPPDATA%\wezterm` log even created) under both `OpenGL` and `Software`, and only `WEZTERM_FRONT_END=WebGpu` brought it up.

One more interaction surfaced on that server: the **Chocolatey installer appends a tab-completion block to `$PROFILE`** (and with CRLF endings). Because `sync-windows.ps1` owns `$PROFILE` whole-file, that block was wiped on the next sync. The tracked profile now carries a guarded `# ---- chocolatey-* ----` marker block (`Import-Module` the choco profile module *if present*), so completion survives the sync, stays LF, and no-ops on hosts without Chocolatey.
