...
1. **Convert at deploy time.** A `run_after` hook renders md → html/txt during `chezmoi apply`. Rejected because Windows has a chezmoi-free update path — `ts-update` in pwsh calls `scripts/sync-windows.ps1`, which never runs the bash hooks — so deploy-time conversion would need a *second*, PowerShell implementation of the markdown converter, and two converters drift.
2. **Generate at authoring time, commit the outputs.** One renderer (`scripts/render-command-reference.sh`, bash + POSIX awk — runs under WSL, Git Bash, macOS, native Linux) produces all four twins; both deployment paths then just copy files exactly as they already do.

We chose (2). The `.txt` twin is a byte-identical copy of the markdown — a copy cannot diverge in content, and markdown already reads fine as plain text. The `.html` twin embeds a `source-sha256:` comment of its markdown source, which makes freshness checkable *without* re-running the converter — that's what lets `scripts/sync-windows.ps1` verify the twins in pure PowerShell. The WSL-side twins keep the `.tmpl` suffix: the converter passes chezmoi `{{ ... }}` lines through verbatim, so the per-OS gates (darwin WezTerm/Homebrew sections, Linux systemd/docker sections) resolve at apply time in all three formats, identically to the `.md`.

Trade-off: a generated artifact in git can be committed stale (edit the md, forget to re-render). Mitigated three ways, all warn-only by design: CLAUDE.md instructs regenerating after every md edit, `run_after_10-check-command-reference.sh` re-renders and byte-compares on every POSIX apply, and `sync-windows.ps1` hash-checks on every Windows sync. None auto-fix — the check hooks run against the deployment clone, and writing into the source tree there would dirty it and break the `ts-update`/`ts-rollback` clean-clone invariant.

The same machinery later gained `docs/command-reference/` — committed per-OS *previews* (md/txt/html for `linux/` ≡ WSL, `macos/`, `windows/`) showing the final post-template content in-repo. Unlike the twins, the previews have no structural justification, only browsability, and they require the renderer to shadow chezmoi's template resolution for the `{{ if eq/ne .chezmoi.os ... }}` forms the file uses — raised as a concern and accepted deliberately (06/2026), at the cost of noisier diffs when the template changes. The shadow is bounded three ways: `resolve_template` hard-fails on any unrecognized template construct (a new template feature breaks the render loudly instead of producing silently-wrong previews), its linux output is byte-verified against real `chezmoi execute-template` output, and the previews live under `docs/**` (chezmoi-ignored) so a stale preview can never reach a machine.
